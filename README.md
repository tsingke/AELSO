...
3. [How the two mechanisms fit together](#3-how-the-two-mechanisms-fit-together)
4. [Benchmark evidence](#4-benchmark-evidence)
5. [Applications beyond benchmarks](#5-applications-beyond-benchmarks)
...
## 3. How the two mechanisms fit together
...
**Core-solution perturbation (CSP)** works only on the current elite set, and that set shrinks steadily as the run progresses. A candidate is produced by moving a *sparse* subset of dimensions — most coordinates are left untouched — and it replaces its parent only if it strictly improves on it. Sparsity keeps the elite's existing structure intact while allowing slow, low-risk gains; strict acceptance means the elite can never drift downhill. The right-hand panel of Figure 2 shows the sequence: elite selection, perturbation of the targeted dimensions, clamping to the search box, and acceptance.

**Dual-exemplar cooperative evolution (DCE)** then handles the rest of the swarm. Individuals are processed from worst to best, and the probability that an individual updates at all rises with its rank, so good solutions are revised more often than poor ones while no individual is ever frozen out entirely. When an update does fire, the individual draws on *two* exemplars rather than one — one chosen as the best of a small randomly sampled subset, the other drawn at random from the elite pool — which keeps a single dominant attractor from swallowing the population. A separate exploration schedule governs whether a non-elite looks toward the leading edge of the swarm as a whole or restricts itself to the elite pool, and that schedule tightens as the budget is spent.
...
AELSO takes the best overall Friedman rank on both suites. Notably, the margin is not built on a handful of easy functions: the win/tie/loss records show consistent advantages across most functions of each suite, including the more strongly coupled and compositional categories. Convergence, parameter-sensitivity and ablation studies accompany the comparison and indicate that the two mechanisms contribute in a complementary rather than redundant way.
...
## 5. Applications beyond benchmarks
